...
public interface M99DailyReportRepository extends JpaRepository<M99DailyReportJpo, Integer> {
    ...
    @Query(value =
        ...
        "WHERE 1= 1\n" +
            "AND (:employeeId IS NULL OR :employeeId = DR.EMPLOYEE_ID)\n" +
            "AND (:projectNumber IS NULL OR (DR.PROJECT_NUMBER LIKE CONCAT('%', :projectNumber, '%')))\n" +
            "AND ((:fromDate IS NULL AND :toDate IS NULL) OR (DR.REGISTER_DATE BETWEEN :fromDate AND :toDate))"
        , nativeQuery = true)
