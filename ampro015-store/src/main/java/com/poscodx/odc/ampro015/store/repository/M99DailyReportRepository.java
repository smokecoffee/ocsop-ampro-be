    ...
    @Query(value =
        ...
        "WHERE 1= 1\n" +
            ...
            "AND ((:fromDate IS NULL AND :toDate IS NULL)\n" +
                " OR ((:toDate IS NULL) AND (:fromDate IS NOT NULL) AND (DR.REGISTER_DATE >= :fromDate))\n" +
                " OR ((:fromDate IS NULL) AND (:toDate IS NOT NULL) AND (DR.REGISTER_DATE <= :toDate))\n" +
                " OR (DR.REGISTER_DATE BETWEEN :fromDate AND :toDate))"
        , nativeQuery = true)
